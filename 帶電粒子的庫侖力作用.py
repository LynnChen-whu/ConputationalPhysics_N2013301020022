...
class Particle(object):
    ...
    def guard(self):
        for i in range(len(self.__position)):
            if ((self.__position[i])**2-(self.__boundry[i])**2)**2<=0.0001:
...
def init():  
    ...
    line6,=axs.plot([],[],[],lw=1,c='m')
    line7,=axs.plot([],[],[],lw=1,c='k')
...
def animate(i): 
    ...
    line6,=axs.plot(X[5],Y[3],Z[5],lw=2,c='m')
    line7,=axs.plot(X[6],Y[3],Z[6],lw=2,c='k')
    return line1,line2,line3,line4,line5,line6,line7
...
#anim1=animation.FuncAnimation(fig,animate,init_func=init,interval=0.1)  
#plt.show()            
